...
type Props={user:{name:string,email:string,role:string};roleLabel:string;allowed:string[];signout:string};
...
function Workspace({allowed,language,setLanguage,tab,setTab,articles,setArticles,notify,currentEmail,currentName,userRole,editArticleItem,setEditArticleItem}:{allowed:string[];language:AdminLanguage;setLanguage:(language:AdminLanguage)=>void;tab:string;setTab:(x:string)=>void;articles:typeof baseArticles;setArticles:(x:typeof baseArticles)=>void;notify:(x:string)=>void;currentEmail:string;currentName:string; userRole?:string; editArticleItem?:any; setEditArticleItem?:(x:any)=>void}){ if(!allowed.includes(tab) && tab !== "डैशबोर्ड" && tab !== "सेटिंग्स") return <section className="workspace"><h2>Access Denied / अनुमति नहीं है</h2></section>; function submit(e:FormEvent<HTMLFormElement>,type:string){e.preventDefault();const f=new FormData(e.currentTarget);if(type==="article"){const title=String(f.get("title"));setArticles([{title,category:String(f.get("category")),author:"न्यूज़ डेस्क",status:String(f.get("status")),views:"—",date:"अभी"},...articles]);notify("नई खबर सेव हुई");setTab("समाचार")}else notify(localize(language,`${type} सेव हुआ`,`${englishLabels[type]??type} saved`))}
 if(tab==="टीम और भूमिकाएँ")return <TeamManager language={language} notify={notify} currentEmail={currentEmail} userRole={userRole}/>;
