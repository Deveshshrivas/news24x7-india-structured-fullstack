...
const nav=["डैशबोर्ड","समाचार","नई पोस्ट","श्रेणियाँ","रिपोर्टर","ब्रेकिंग न्यूज़","ऑडियो हाइलाइट्स","विज्ञापन","मीडिया लाइब्रेरी","टिप्पणियाँ","टीम और भूमिकाएँ","वेबसाइट कंटेंट"];
const englishLabels:Record<string,string>={"डैशबोर्ड":"Dashboard","समाचार":"News","नई पोस्ट":"New post","श्रेणियाँ":"Categories","रिपोर्टर":"Reporters","ब्रेकिंग न्यूज़":"Breaking news","ऑडियो हाइलाइट्स":"Audio highlights","विज्ञापन":"Advertising","मीडिया लाइब्रेरी":"Media library","टिप्पणियाँ":"Comments","टीम और भूमिकाएँ":"Team and roles","सेटिंग्स":"Settings", "वेबसाइट कंटेंट":"Website Content", "सुपर एडमिन":"Super admin","एडमिन":"Admin","मुख्य संपादक":"Editor in chief","विज्ञापन प्रबंधक":"Ad manager"};
...
function Workspace({language,setLanguage,tab,setTab,articles,setArticles,notify,currentEmail,currentName}:{language:AdminLanguage;setLanguage:(language:AdminLanguage)=>void;tab:string;setTab:(x:string)=>void;articles:typeof baseArticles;setArticles:(x:typeof baseArticles)=>void;notify:(x:string)=>void;currentEmail:string;currentName:string}){ function submit(e:FormEvent<HTMLFormElement>,type:string){e.preventDefault();const f=new FormData(e.currentTarget);if(type==="article"){const title=String(f.get("title"));setArticles([{title,category:String(f.get("category")),author:"न्यूज़ डेस्क",status:String(f.get("status")),views:"—",date:"अभी"},...articles]);notify("नई खबर सेव हुई");setTab("समाचार")}else notify(localize(language,`${type} सेव हुआ`,`${englishLabels[type]??type} saved`))}
 ...
 if(tab==="समाचार")return <NewsManager language={language} mode="list" setTab={setTab} notify={notify}/>;
