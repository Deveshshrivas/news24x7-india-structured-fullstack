...
export default function AudioHighlightsManager({notify, language}:{notify:(x:string)=>void; language: "hi" | "en"}){
 ...
 async function add(e:FormEvent<HTMLFormElement>){
  ...
  if(!(file instanceof File)||!file.size||!file.name.toLowerCase().endsWith('.mp3')){setError(text('कृपया एक MP3 फ़ाइल चुनें।', 'Please select an MP3 file.'));return}
  if(file.size>25*1024*1024){setError(text('MP3 25 MB या उससे छोटा होना चाहिए।', 'MP3 must be 25 MB or smaller.'));return}
  setUploading(true);
  try{await readApiResponse(await fetch('/api/backend/audio',{method:'POST',body:data}));form.reset();await load();notify(text('MP3 न्यूज़ अपलोड हुई', 'MP3 news uploaded'))}
 ...
 async function remove(x:Item){if(!confirm(text(`“${x.title}” हटाएँ?`, `Remove “${x.title}”?`)))return;try{await readApiResponse(await fetch(`/api/backend/audio/${x.id}`,{method:'DELETE'}));await load();notify(text('MP3 हटाई गई', 'MP3 removed'))}catch(e){setError(e instanceof Error?e.message:'Delete failed')}}
 return <section className="workspace"><div className="workspaceHead"><div><h2>{text("न्यूज़ MP3 लाइब्रेरी", "News MP3 Library")}</h2><p>{text("जितनी चाहें MP3 रखें; क्रम में पहली 10 सक्रिय खबरें होमपेज पर चलेंगी", "Keep as many MP3s as you want; the top 10 active news in order will play on the homepage")}</p></div><span className="liveCount">{text(`${items.filter(x=>x.active).length} सक्रिय / ${items.length} कुल`, `${items.filter(x=>x.active).length} active / ${items.length} total`)}</span></div><div className="audioSettings"><strong>{text("MP3 अपलोड पर कोई संख्या सीमा नहीं", "No limit on MP3 uploads")}</strong><span>{text("हर फाइल अधिकतम 25 MB • केवल MP3 • होमपेज प्लेलिस्ट: पहली 10 सक्रिय", "Each file max 25 MB • MP3 only • Homepage playlist: top 10 active")}</span></div><form className="audioAddForm mp3Upload" onSubmit={add}><input required name="title" placeholder={text("न्यूज़ का शीर्षक", "News title")}/><label className="audioFile">{text("MP3 चुनें", "Select MP3")}<input required name="audio" type="file" accept="audio/mpeg,.mp3"/></label><button className="primary" disabled={uploading}>{uploading?text("अपलोड हो रहा है…", "Uploading..."):text("＋ MP3 अपलोड करें", "＋ Upload MP3")}</button>{error&&<p className="uploadError">{error}</p>}</form><div className="audioAdminList">{!items.length&&<div className="audioEmpty"><b>{text("अभी कोई MP3 नहीं है", "No MP3s yet")}</b><span>{text("ऊपर से पहली न्यूज़ ऑडियो अपलोड करें।", "Upload your first news audio from above.")}</span></div>}{items.map((x,i)=><article className={x.active?"":"disabledHighlight"} key={x.id}><b>{i+1}</b><div><span>{(x.size/1024/1024).toFixed(1)} MB • {x.filename}{x.active&&i<10?text(" • होमपेज टॉप 10", " • Homepage Top 10"):""}</span><h3>{x.title}</h3></div><div><button onClick={()=>play(x)}>{playingId === x.id ? "⏸ " + text("रोकें", "Pause") : "▶ " + text("सुनें", "Listen")}</button><button onClick={()=>patch({id:x.id,active:!x.active})}>{x.active?text("निष्क्रिय करें", "Deactivate"):text("सक्रिय करें", "Activate")}</button><button disabled={i===0} onClick={()=>patch({id:x.id,direction:"up"})}>↑</button><button disabled={i===items.length-1} onClick={()=>patch({id:x.id,direction:"down"})}>↓</button><button onClick={()=>remove(x)}>{text("हटाएँ", "Remove")}</button></div></article>)}</div></section>
}
