...
export default function AudioHighlightsManager({notify, language}:{notify:(x:string)=>void; language: "hi" | "en"}){
 ...
 async function add(e:FormEvent<HTMLFormElement>){
  ...
  if(!(file instanceof File)||!file.size||!(/\.(mp3|mpeg|wav|ogg|aac|m4a|wma|flac|webm)$/i.test(file.name))){setError(text('कृपया एक ऑडियो फ़ाइल चुनें (MP3, MPEG, WAV, OGG, AAC, M4A, FLAC)', 'Please select an audio file (MP3, WAV, OGG, AAC, M4A, FLAC)'));return}
  if(file.size>100*1024*1024){setError(text('ऑडियो 100 MB या उससे छोटा होना चाहिए।', 'Audio must be 100 MB or smaller.'));return}
  setUploading(true);
