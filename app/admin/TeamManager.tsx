...
export default function TeamManager({notify,currentEmail,language}:{notify:(message:string)=>void;currentEmail:string;language:"hi"|"en"}){
  ...
  async function load(){
    ...
    try{
      ...
      setError(caught instanceof Error?caught.message:text("उपयोगकर्ता सूची लोड नहीं हुई", "Failed to load user list"));
    }finally{
      setLoading(false);
    }
  }
  ...
  useEffect(()=>{
    ...
    fetch("/api/backend/users",{cache:"no-store"})
      ...
      .catch(caught=>{if(!cancelled)setError(caught instanceof Error?caught.message:text("उपयोगकर्ता सूची लोड नहीं हुई", "Failed to load user list"))})
  ...
  return <section className="workspace">
    <div className="workspaceHead"><div><h2>{text("टीम और भूमिकाएँ", "Team and Roles")}</h2><p>{text("केवल सुपर एडमिन नए एडमिन बना सकता है और सभी उपयोगकर्ताओं की भूमिका या स्थिति बदल सकता है।", "Only Super Admin can create new admins and change roles or status of all users.")}</p></div><span className="superAdminOnly">{text("सुपर एडमिन नियंत्रण", "Super Admin Control")}</span></div>
    <form className="teamAdd" onSubmit={create}>
      ...
      <label>{text("भूमिका", "Role")}<select name="role" defaultValue="admin">{Object.entries(labels).map(([value,label])=><option key={value} value={value}>{label}</option>)}</select></label>
