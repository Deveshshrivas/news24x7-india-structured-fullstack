...
function output(item:Record<string,unknown>){return{id:String(item._id),reporterId:item.reporter_id??'',name:item.name,designation:item.designation,phone:item.phone,email:item.email,country:item.country??'',state:item.state??'',city:item.city??'',pincode:item.pincode??'',address:item.address,active:item.active??true,photoUrl:item.photo_file_id?`/reporters/${item._id}/photo`:null,updatedAt:item.updated_at instanceof Date?item.updated_at.toISOString():null}}
...
reportersRouter.post("/",requirePermission("reporters"),upload.single("photo"),asyncRoute(async(request,response)=>{const body=reporterSchema.parse(request.body);if(await db.collection("reporters").findOne({email:body.email.toLowerCase()}))throw new AppError(409,"Reporter email already exists");if(body.reporter_id&&await db.collection("reporters").findOne({reporter_id:body.reporter_id}))throw new AppError(409,"Reporter ID already exists");if(!body.reporter_id)delete body.reporter_id;const now=new Date(),document:Record<string,unknown>={...body,email:body.email.toLowerCase(),created_at:now,updated_at:now};if(request.file){document.photo_file_id=await savePhoto(request.file);document.photo_content_type=request.file.mimetype}const inserted=await db.collection("reporters").insertOne(document);broadcastNotification(`New reporter added: ${document.name}`, `/`, ["super_admin", "admin", "editor"]);response.status(201).json({item:output({...document,_id:inserted.insertedId})})}));
reportersRouter.patch("/:itemId",requirePermission("reporters"),upload.single("photo"),asyncRoute(async(request,response)=>{const id=objectId(routeParam(request.params.itemId)),existing=await db.collection("reporters").findOne({_id:id});if(!existing)throw new AppError(404,"Reporter not found");const body=reporterSchema.parse(request.body),email=body.email.toLowerCase();if(await db.collection("reporters").findOne({_id:{$ne:id},email:{$regex:`^${escapeRegex(email)}$`,$options:"i"}}))throw new AppError(409,"Reporter email already exists");if(body.reporter_id&&await db.collection("reporters").findOne({_id:{$ne:id},reporter_id:body.reporter_id}))throw new AppError(409,"Reporter ID already exists");if(!body.reporter_id)delete body.reporter_id;const updates:Record<string,unknown>={...body,email,updated_at:new Date()};let previousPhoto=existing.photo_file_id;if(request.file){updates.photo_file_id=await savePhoto(request.file);updates.photo_content_type=request.file.mimetype}await db.collection("reporters").updateOne({_id:id},{$set:updates});if(request.file&&previousPhoto)await reporterPhotos.delete(previousPhoto).catch(()=>undefined);broadcastNotification(`Reporter updated: ${updates.name || existing.name}`, `/`, ["super_admin", "admin", "editor"]);response.json({item:output({...existing,...updates})})}));
reportersRouter.delete("/:itemId",requirePermission("reporters"),asyncRoute(async(request,response)=>{const row=await db.collection("reporters").findOneAndDelete({_id:objectId(routeParam(request.params.itemId))});if(!row)throw new AppError(404,"Reporter not found");if(row.photo_file_id)await reporterPhotos.delete(row.photo_file_id).catch(()=>undefined);broadcastNotification(`Reporter removed: ${row.name}`, `/`, ["super_admin", "admin", "editor"]);response.json({ok:true})}));
