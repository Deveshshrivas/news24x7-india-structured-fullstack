...
reportersRouter.patch("/:itemId",requirePermission("reporters"),upload.single("photo"),asyncRoute(async(request,response)=>{const id=objectId(routeParam(request.params.itemId)),existing=await db.collection("reporters").findOne({_id:id});if(!existing)throw new AppError(404,"Reporter not found");const body=reporterSchema.parse(request.body),email=body.email.toLowerCase();if(await db.collection("reporters").findOne({_id:{$ne:id},email:{$regex:`^${escapeRegex(email)}$`,$options:"i"}}))throw new AppError(409,"Reporter email already exists");if(body.reporter_id&&await db.collection("reporters").findOne({_id:{$ne:id},reporter_id:body.reporter_id}))throw new AppError(409,"Reporter ID already exists");if(!body.reporter_id)delete body.reporter_id;const updates:Record<string,unknown>={...body,email,updated_at:new Date()};let previousPhoto=existing.photo_file_id;if(request.file){updates.photo_file_id=await savePhoto(request.file);updates.photo_content_type=request.file.mimetype}await db.collection("reporters").updateOne({_id:id},{$set:updates});if(request.file&&previousPhoto)await reporterPhotos.delete(previousPhoto).catch(()=>undefined);response.json({item:output({...existing,...updates})})}));
