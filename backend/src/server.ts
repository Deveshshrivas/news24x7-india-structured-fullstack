...
const errorHandler:ErrorRequestHandler=(error,_request,response,_next)=>{
  ...
  if(error instanceof multer.MulterError){response.status(error.code==="LIMIT_FILE_SIZE"?413:400).json({detail:error.code==="LIMIT_FILE_SIZE"?"MP3 must be 25 MB or smaller":error.message});return}
