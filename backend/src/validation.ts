...
export const reporterSchema=z.object({reporter_id:z.string().trim().max(40).regex(/^[A-Za-z0-9_-]*$/, 'Reporter ID can contain letters, numbers, hyphens and underscores').transform(value=>value.toUpperCase()).optional(),name:z.string().trim().min(2).max(80),designation:z.string().trim().min(2).max(100),phone:z.string().trim().min(7).max(24),email:z.email(),country:z.string().trim().max(50).optional(),state:z.string().trim().max(50).optional(),city:z.string().trim().max(50).optional(),pincode:z.string().trim().max(20).optional(),address:z.string().trim().min(5).max(300),active:z.preprocess(value=>value==="true"||value===true,z.boolean()).default(true)});
